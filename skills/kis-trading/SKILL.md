...
| # | 항목 | 확인 내용 |
|---|---|---|
| 1 | 이벤트/뉴스 스캔 | 최근 1~2주 뉴스 + **임박 이벤트**(실적발표·FOMC·CPI·OPEC·선거·지정학) + 종목경고(`toss_market kind: "warnings"` 또는 뉴스 검색) |
...
| 6 | 이벤트 대응 사전 설정 | 반대 이벤트 발생 시 손절/헤지 계획 — 토스 키 있으면 **OCO 조건주문(손절+목표 동시)** 으로 자동화 권장 (KIS는 조건주문 없음). **실시간 감시는 /kis-watch** (subagent 대신 프로젝트 내장 헤드리스 워치 — 조건 충족 시 알림/사전 승인 주문) |
