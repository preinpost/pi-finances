...
<ul class="risk-list">
  ...
  <li><strong>이벤트 민감도: 유가·중동 이벤트(이란 종전 등) 시 에너지 ETF 급락 위험</strong> — 반대 이벤트 발생 시 손절/헤지 계획</li>
  <li>$297 이탈 시 구조적 하락 가속 → $275 → $271</li>
</ul>
```

> 리스크 항목에 **이벤트 민감도**(반대 방향 이벤트 발생 시 급락 위험 + 대응 계획)를 반드시 포함 — XLE 사례: 이란 종전 같은 지정학 이벤트에 포지션이 즉시 손실될 수 있는 종목은 그 위험과 사전 대응을 명시한다.
