/**
 * src/cache.ts — TTL 메모리 캐시 (API 호출 절약 — 레이트리밋 대응).
 *
 * 엔드포인트별 TTL은 roles/coingecko.ts에서 인스턴스별로 설정한다:
 *   price 15s / chart 60s / market 2m / coin 10m / search 10m.
 *
 * 환경변수: COINGECKO_DISABLE_CACHE=1 이면 cached()가 캐시 없이 직접 실행.
 */
 ...
 * COINGECKO_DISABLE_CACHE=1 이면 캐시를 건너뛰고 fn()을 직접 호출한다.
 */
export async function cached<T>(cache: TtlCache, key: string, fn: () => Promise<T>): Promise<T> {
	if (process.env.COINGECKO_DISABLE_CACHE === "1") return fn();
