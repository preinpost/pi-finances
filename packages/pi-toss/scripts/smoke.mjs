#!/usr/bin/env node
/**
 * CI/로컬 스모크 테스트 — pi-toss 확장이 계약된 툴/커맨드를 등록하는지 검증.
 *
 * 실행 (패키지 루트에서):
 *   node --experimental-transform-types scripts/smoke.mjs
 *
 * 툴 name은 pi-kis 0.2.x와 하위 호환 계약 — 목록이 바뀌면 의도된 breaking일
 * 때만 이 파일도 함께 갱신한다.
 */
...
if (failures.length > 0) {
	console.error(`[pi-toss smoke] FAIL\n${failures.join("\n")}`);
	process.exit(1);
}
console.log(`[pi-toss smoke] PASS — 툴 ${tools.length}개, 커맨드 ${commands.length}개`);
