#!/usr/bin/env node
/**
 * CI/로컬 스모크 테스트 — pi-finnhub 확장이 계약된 툴/커맨드를 등록하는지 검증.
 *
 * 실행 (패키지 루트에서):
 *   node --experimental-transform-types scripts/smoke.mjs
 *
 * 툴 name은 공개 계약 — 목록이 바뀌면 의도된 breaking일 때만 이 파일도
 * 함께 갱신한다.
 */
const EXPECTED_TOOLS = ["finnhub_price", "finnhub_chart", "finnhub_news", "finnhub_fundamentals"];
const EXPECTED_COMMANDS = ["finnhub-key", "finnhub-status"];
...
if (failures.length > 0) {
	console.error(`[pi-finnhub smoke] FAIL\n${failures.join("\n")}`);
	process.exit(1);
}
console.log(`[pi-finnhub smoke] PASS — 툴 ${tools.length}개, 커맨드 ${commands.length}개`);
