#!/usr/bin/env node
/**
 * CI/로컬 스모크 테스트 — pi-twelve-data 확장이 계약된 툴/커맨드를 등록하는지 검증.
 *
 * 실행 (패키지 루트에서):
 *   node --experimental-transform-types scripts/smoke.mjs
 */
const EXPECTED_TOOLS = ["twelve_price", "twelve_chart", "twelve_search", "twelve_exchange_rate"];
const EXPECTED_COMMANDS = ["twelve-key", "twelve-status"];
...
if (failures.length > 0) {
	console.error(`[pi-twelve-data smoke] FAIL\n${failures.join("\n")}`);
	process.exit(1);
}
console.log(`[pi-twelve-data smoke] PASS — 툴 ${tools.length}개, 커맨드 ${commands.length}개`);
