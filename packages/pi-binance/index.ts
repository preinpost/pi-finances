 ...
 *   src/agent/        — binance_* 6툴, /binance-key, /binance-status
 *
 * Tools:
 *   - binance_price    현재가·24h 변동 (현물/USDT-M, 키 없이 가능)
 *   - binance_chart    캔들 + 공용 지표 (키 없이 가능)
 *   - binance_account  현물 잔고 / 선물 잔고·포지션 (서명)
 *   - binance_order    주문 생성 (서명 — 사용자 확인 후에만)
 *   - binance_orders   미체결/상세/취소 (서명 — 취소는 사용자 확인 후에만)
 *   - binance_futures  펀딩·마크가·미결제약정·레버리지·마진타입 (선물 전용)
 *
 * Commands:
