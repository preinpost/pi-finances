#!/usr/bin/env node
/**
 * CI/로컬 스모크 테스트 — pi-naver-news 확장이 계약된 툴/커맨드를 등록하는지 검증.
 *
 * 실행 (패키지 루트에서):
 *   node --experimental-transform-types scripts/smoke.mjs
 */
const EXPECTED_TOOLS = ["naver_news_search"];
const EXPECTED_COMMANDS = ["naver-news-key", "naver-news-status"];
...
if (failures.length > 0) {
	console.error(`[pi-naver-news smoke] FAIL\n${failures.join("\n")}`);
	process.exit(1);
}
console.log(`[pi-naver-news smoke] PASS — 툴 ${tools.length}개, 커맨드 ${commands.length}개`);
